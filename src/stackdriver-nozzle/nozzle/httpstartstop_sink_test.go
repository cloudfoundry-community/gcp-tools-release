...
func (ta testApp) RequestCount(instanceIndex int) int {
	key := telemetry.MapKey(map[string]string{
		"index":           ta.Index(),
...
func (ta testApp) ResponseClass(class string, instanceIndex int) int {
	key := telemetry.MapKey(map[string]string{
		"index":           ta.Index(),
		"applicationPath": makePath(ta.AppInfo()),
		"instanceIndex":   strconv.Itoa(instanceIndex),
	...
	if ctr, ok := responseClass.Get(key).(*telemetry.Counter); ok {
		return int(ctr.Value())
	}
	// Zero is a perfectly valid counter value, -1 not so much.
	// Returning a separate error here is inconvenient.
...
var _ = Describe("HttpSink", func() {
	...
	It("increments counters for reqeusts", func() {
		...
		Expect(testApps[0].ResponseClass("2xx", 0)).To(Equal(20))
		Expect(testApps[0].ResponseClass("5xx", 0)).To(Equal(-1))
		...
		Expect(testApps[0].ResponseClass("2xx", 1)).To(Equal(18))
		Expect(testApps[0].ResponseClass("5xx", 1)).To(Equal(2))

		Expect(testApps[0].RequestCount(2)).To(Equal(-1))
		Expect(testApps[0].ResponseCode(200, 2)).To(Equal(-1))
		Expect(testApps[0].ResponseClass("2xx", 2)).To(Equal(-1))
		...
		Expect(testApps[1].ResponseClass("2xx", 0)).To(Equal(15))
		Expect(testApps[1].ResponseClass("4xx", 0)).To(Equal(10))

		Expect(testApps[2].RequestCount(0)).To(Equal(8))
		Expect(testApps[2].ResponseCode(302, 0)).To(Equal(8))
		Expect(testApps[2].ResponseClass("3xx", 0)).To(Equal(8))
	})
})
