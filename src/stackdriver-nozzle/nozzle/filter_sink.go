...
var (
	filteredEvents    *telemetry.Counter
	blacklistedEvents *telemetry.Counter
)

func init() {
	filteredEvents = telemetry.NewCounter(telemetry.Nozzle, "filter_sink.filtered_events")
...
func (fs *filter) isBlacklisted(event *events.Envelope) bool {
	if fs.blacklist.Match(event) && !fs.whitelist.Match(event) {
		return true
	}
	return false
}
...
func (fs *filter) Receive(event *events.Envelope) {
	if !fs.enabled[event.GetEventType()] {
		filteredEvents.Increment()
		return
	}
	if fs.isBlacklisted(event) {
