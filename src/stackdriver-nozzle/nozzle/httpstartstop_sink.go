...
var (
	...
	responseClass *telemetry.CounterMap

	defaultLabels = []string{"index", "applicationPath", "instanceIndex"}
)

func init() {
	// The telemetry sink handles the "foundation" label for us \o/
	...
	responseClass = telemetry.NewCounterMap(httpPrefix, "response_class",
		append(defaultLabels, "class")...)
}
...
func (sink *httpSink) Receive(envelope *events.Envelope) {
	...
	if envelope.GetJob() != "router" || labels["applicationPath"] == "" {
	...
	class := fmt.Sprintf("%dxx", envelope.GetHttpStartStop().GetStatusCode()/100)
	if rcc, err := responseClass.Counter(append(labelValues, class)...); err == nil {
		rcc.Increment()
	} else {
		sink.logger.Error("httpSink.Receive", fmt.Errorf("incrementing responseClass: %v", err))
	}
}
